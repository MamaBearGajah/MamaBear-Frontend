...
const HomeBanner = () => {
  ...
  React.useEffect(() => {
    const fetchBannerData = async () => {
      try {
        ...
        // Backend TransformInterceptor selalu wrap: { success: true, data: [...] }
        // Handle semua kemungkinan shape untuk safety:
        // - { success, data: [...] }   → ambil data
        // - { data: [...] }            → ambil data
        // - [...]                      → langsung pakai
        const raw = res.data;
        let banners: any[] = [];
        ...
        const mappedBanners: CarouselBannerItem[] = banners.map((item: any, index: number) => ({
  ...
  React.useEffect(() => {
    if (!api) return;
    const autoplay = window.setInterval(() => api.scrollNext(), 5000);
  ...
  if (loading) {
    return <div className="h-[460px] w-full animate-pulse bg-gray-200" />;
  }
                          ...
                          <div className="relative z-10 max-w-full text-left font-sans md:max-w-[560px]">
                            <div className="banner-text-group translate-x-[0.1cm]">
                              <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-[#FACBD8] px-4 py-1.5 text-sm font-semibold text-[#6C4735]">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 flex-shrink-0">
                                  <path d="M12 2.5l1.902 4.357 4.812.7-3.478 3.135.82 4.79L12 14.77l-4.056 2.712.82-4.79L5.286 7.557l4.812-.7L12 2.5z" fill="#6C4735" />
                                </svg>
            ...
            <div className="absolute inset-0 z-10 flex items-center justify-start px-4 text-left">
              <div className="max-w-[88%]">
                <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#FACBD8] px-4 py-1.5 text-xs font-semibold text-[#6C4735]">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 flex-shrink-0">
                    <path d="M12 2.5l1.902 4.357 4.812.7-3.478 3.135.82 4.79L12 14.77l-4.056 2.712.82-4.79L5.286 7.557l4.812-.7L12 2.5z" fill="#6C4735" />
                  </svg>
                  <span className="text-xs leading-none">{bannerData[mobileIndex].label}</span>
                </div>
